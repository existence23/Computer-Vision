function batchToVisualWords(numCores) 
    ...
    %********************************************************************
    %to seperate harris with random, I build a Harris folder and Random
    %folder in the data folder, I store these data folders in Harris and
    %Random seperately, then the result mat file would store into the
    %different folder according to the different map built ways.
    %********************************************************************

	%load the files and texton dictionary
	load('../data/traintest.mat','all_imagenames','mapping');
    %i set the harris wordmap and the random wordmap in different folders
    %so i changed the position here
	load('../matlab/dictionaryHarris.mat','filterBank','harrisDictionary');
    %load('../matlab/dictionaryRandom.mat','filterBank,'randomDictionary');
